...
class FileIO extends FileIOInterface {
  ...
  override def load(): (Option[GridInterface], Int) = {
    ...
    Try {
      ...
      val source: String = scala.io.Source.fromURL("http://localhost:8887/loadGrid/2").mkString
  ...
  override def save(grid: GridInterface): Unit = {
    ...
    val url = "http://localhost:8887/?save=saveGrid&gridJson={\"grid\":20}"
    println(url)
    print(scala.io.Source.fromURL(url).mkString)
